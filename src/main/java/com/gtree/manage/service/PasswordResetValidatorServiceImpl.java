 ...
 * 
 * @author SrinivasuluP
 * This validator implementation does the necessary validations on the input request
 *  - Checks if the inputs are provided
 *  - Checks if the inputs are in standard acceptable format
 *  - Checks if the user exists based on the inputs
 */
@Service
public class PasswordResetValidatorServiceImpl implements PasswordResetValidatorService {
	...
	@Autowired
	public PasswordResetValidatorServiceImpl(UserPwdRepository userRepo) {
		this.userRepo = userRepo;
	}
	 
	/**
	 * This method does the crux of input validations.
	 * REGEX is used to validate the format of email
	 * Checks if the inputs are correct
	 * Checks if the user exists based on the inputs provided
	 */
	@Override
	public boolean passwordResetRequestValidator(PasswordResetRequest request) {
		...
		if(StringUtils.isEmpty(request.getNewPassword()) || StringUtils.isEmpty(request.getConfirmPassword()) || 
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "New password and confirmation password should match");	
		}
		else if(!pattern.matcher(request.getEmail()).matches()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid email");	
		} else if(userRepo.findByEmail(request.getEmail()) == null) {
